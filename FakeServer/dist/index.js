...
class Route {
    ...
    query(req, res) {
        const response = this.#callback(req, res);
...
class Router {
    constructor(Routers) {
        this.#initRouterMap(Routers);
    }
    #RouterMap = new Map();
    #RouteMatchers = [];
    #initRouterMap(Routers) {
        Object.entries(Routers).forEach(([key, value]) => {
            if (value instanceof Function) this.addRoute(key, value);
        });
    ...
    getServerResult(path, options = {}) {
        ...
        const target = this.#findRoute(path);
