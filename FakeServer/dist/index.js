...
function parseURL(url, strictMode = false) {
...
const BODY = Symbol('Body internals');
const RESPONSE = Symbol('Response internals');
...
class Body {
     ...
     */
    async arrayBuffer() {
        const blob = await consumeBody(this[BODY]);
...
const extractContentType = (body, request) => {
    ...
    if (body instanceof FormData) {
        return `multipart/form-data; boundary=${request[BODY].boundary}`;
    }
 ...
 */
const Response$1 = (globalThis.window && globalThis.window.Response) || class Null {};
/**
 * Response class
 *
 * @param   Stream  body  Readable stream
 ...
 * @return  Void
 */
class fakeResponse extends Body {
    ...
    get redirected() {
        return this[RESPONSE].counter > 0;
    }
    ...
    clone() {
        ...
        return new Response$1(this.body, {
            type: this.type,
            url: this.url,
            status: this.status,
...
const config$1 = { proxy: null, silent: false };
let realFetch;
...
async function fakeFetch(url, options = {}) {
    ...
    if (window.fetch.$mock === true) {
        // ! 传入初始参数
        const result = await config$1.proxy(url, options);
        if (result) {
            if (!config$1.silent) console.warn('fetch: mock代理中');
            const { body, options = {} } = result;
            return new fakeResponse(body, options);
        }
    }

    if (!config$1.silent) console.warn('这次 fetch 未使用 mockjs');
    return realFetch(url, options);
}

// 代理出口
function mockFetch({ proxy, silent = false }) {
    if (proxy instanceof Function) config$1.proxy = proxy;
    config$1.silent = silent;
    // 代理 fetch 的初始化函数
...
export { FakeServer, mockFetch, mockXHR };
